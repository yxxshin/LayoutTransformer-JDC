...
def sample(model, x, steps):
    model.eval()
    b, t, dim = x.size()  # dim should be 20
    ...
    for _ in range(steps - t):
        ...
        processed_logits = model(x_unfinished)  # Should output [b, t, 5]
        
        # Convert to 20-dimensional format
        ix = transfer_to_onehot(processed_logits[:, -1, :].unsqueeze(1))

        # Create new step tensor
        new_step = torch.zeros((b, 1, dim), device=x.device)
        new_step[:, :, -1] = 1  # Default to PAD token

        for idx, orig_idx in enumerate(unfinished_indices):
            # Check for EOS token (index 14 in one-hot)
            if torch.argmax(ix[idx, 0, 4:]) == 14:
                finished_sequences[orig_idx] = True

            # Validate coordinates
            coords = ix[idx, 0, :4]
            if (coords < 0.0).any() or (coords > 1.0).any():
                finished_sequences[orig_idx] = True
                new_step[orig_idx] = torch.tensor(
                    [0.0]*4 + [0]*13 + [0, 1, 0],  # EOS token
                    device=x.device
                )
            else:
...
def trim_tokens(tokens, bos=13.0, eos=14.0, pad=15.0):
    categories = tokens[:, 0]
...
def transfer_to_onehot(x):
    ...
    x_onehot = F.one_hot(x_categories, num_classes=16)
