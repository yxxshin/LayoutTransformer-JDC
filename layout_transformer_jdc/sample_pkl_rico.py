...
from utils_rico import sample, trim_tokens
...
def get_args():
    ...
    parser.add_argument("--disc_dim", default=16, type=int)
...
def convert_baseline_to_model_input(data, device):
    ...
    sequence = torch.zeros(batch_size, seq_length, 20, device=device)
    ...
    bos_token = torch.tensor(
        [0.0] * 4 + [0] * 13 + [1, 0, 0], dtype=torch.float32, device=device
...
def main():
    ...
    if args.dataset_type == "test":
        dataset = get_dataset(
            "rico",
            "test",
            data_path="/home/yxxshin/Desktop/CVLab/LayoutTransformer-JDC/data/dataset/rico",
        )
